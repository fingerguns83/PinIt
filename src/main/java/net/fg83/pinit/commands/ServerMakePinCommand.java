...
public class ServerMakePinCommand implements CommandExecutor {
    PinIt plugin;

    public ServerMakePinCommand(PinIt plugin) {
        this.plugin = plugin;
    }
    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (sender instanceof Player){
            ...
            if (args.length < 5){
                plugin.sendPinItMessage(player, "Missing info. Usage \"/setserverpin <world> <x> <y> <z> <name>\"", true);
            }
            else {
                String name = "";
                String pinWorld = "";
                ...
                for (int i = 0; i < args.length; i++) {
                    try {
                        ...
                        plugin.sendPinItMessage(player, "One of your coordinates is not a number. Usage \"/setserverpin <world> <x> <y> <z> <name>\"", true);
                        return false;
                    }
                }
                ...
                Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
                    new Pin(finalName.trim(), location, plugin).store();
