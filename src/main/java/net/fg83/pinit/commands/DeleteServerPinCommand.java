...
public class DeleteServerPinCommand implements CommandExecutor {
    PinIt plugin;
    public DeleteServerPinCommand(PinIt plugin) { this.plugin = plugin; }
    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        try {
            ...
            plugin.sendPinItMessage(player, "Invalid argument! Usage: \"/delserverpin <pinId>\"", true);
            return false;
        }
        if (pinId < 1){
            plugin.sendPinItMessage(player, "Invalid argument! Usage: \"/delserverpin <pinId>\"", true);
        ...
        Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
            try {
                String input = "DELETE FROM server WHERE id = ?";
